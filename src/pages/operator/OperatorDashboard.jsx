...
const Dashboard = () => {
  ...
  const statusOptions = ["Semua Status", "Proses", "Terbit", "Reject", "Revoke"];
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const uniqueFaculties = [...new Set(rows.map((item) => item.fakultas).filter((f) => f && f !== "-"))];
        setFakultasOptions(["Semua Fakultas", ...uniqueFaculties]);

        // Ekstrak Tahun Unik
        const uniqueYears = [...new Set(rows.map((item) => item.tahun_lulus?.toString()).filter((t) => t && t !== "-"))].sort((a, b) => Number(b) - Number(a));
  ...
  const filteredData = tableData.filter((item) => {
    ...
    const searchableText = [item.nama, item.nim, item.prodi, item.fakultas, item.tahun_lulus, item.status, item.batch].join(" ").toLowerCase();
    ...
    const matchesStatus = selectedStatus === "Semua Status" || normalizeStatus(item.status) === normalizeStatus(selectedStatus);
  ...
  useEffect(() => { setCurrentPage(1); }, [searchQuery, selectedFakultas, selectedStatus, selectedTahun]);

  // ==================== 🔥 SINKRONISASI STATISTIK (DARI TABEL) 🔥 ====================
  // Kita pakai filteredData supaya kalau dropdown diganti, angkanya juga ikut ganti!
  const dynamicStats = {
    terbit: filteredData.filter(d => normalizeStatus(d.status) === 'terbit').length,
    proses: filteredData.filter(d => normalizeStatus(d.status) === 'proses').length,
    rejected: filteredData.filter(d => normalizeStatus(d.status) === 'reject').length,
    revoked: filteredData.filter(d => normalizeStatus(d.status) === 'revoke').length,
  };

  const verificationChartData = {
    labels: ["Terbit", "Proses", "Reject", "Revoke"],
    data: [dynamicStats.terbit, dynamicStats.proses, dynamicStats.rejected, dynamicStats.revoked],
    ...
    return pages.map((page, index) => (
      <button
        ...
        className={`w-8 h-8 flex items-center justify-center rounded text-xs font-bold shadow-sm transition-colors ${
          page === currentPage ? "bg-[#117065] text-white" : page === "..." ? "bg-transparent text-gray-400 cursor-default shadow-none" : "bg-[#E5E7EB] text-gray-500 hover:bg-gray-300"
        }`}
      >
        {page}
      </button>
    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h2 className="font-bold text-gray-800 mb-4 text-lg">Status Data Ijazah Tahun {selectedTahun === "Semua Tahun" ? new Date().getFullYear() : selectedTahun}</h2>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <tbody className="text-gray-700">
              {currentData.length > 0 ? (
                currentData.map((row, i) => (
                  <tr key={row.id || row.id_mahasiswa || i} className="border-t border-gray-200 hover:bg-gray-50">
                    <td className="px-4 py-4 text-center font-semibold">{indexOfFirstItem + i + 1}.</td>
                    ...
                    <td className="px-4 py-4 text-center font-semibold text-gray-700 truncate">{row.nim || "-"}</td>
                    <td className="px-4 py-4 text-center text-gray-600 font-semibold whitespace-nowrap truncate">{row.fakultas || "-"}</td>
                    <td className="px-4 py-4 text-center text-gray-600 font-semibold truncate">{row.prodi || "-"}</td>
                    <td className="px-4 py-4 text-center font-semibold text-gray-700">{row.tahun_lulus || "-"}</td>
                    <td className="px-4 py-4 text-center">
                      <span className={`inline-block min-w-[86px] px-4 py-1.5 rounded-full text-xs font-bold ${getBadgeColor(row.status)}`}>{getBadgeLabel(row.status)}</span>
                    </td>
                  </tr>
                ))
              ) : (
                <tr><td colSpan="7" className="px-4 py-8 text-center text-gray-400">Data tidak ditemukan.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="p-6 flex flex-col md:flex-row justify-between items-center gap-4 border-t border-gray-100">
          <p className="text-xs text-gray-400">Menampilkan {currentData.length} dari {filteredData.length} Data</p>
          {totalPages > 1 && (
            <div className="flex items-center gap-1.5">
              <button type="button" onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-black font-bold disabled:opacity-50">{"<"}</button>
              {renderPaginationButtons()}
              <button type="button" onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-black font-bold disabled:opacity-50">{">"}</button>
            </div>
          )}
        </div>
      </div>
