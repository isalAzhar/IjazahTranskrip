...
const ManajemenData = () => {
  ...
  const tahunList = ["Semua Tahun", "2021", "2022", "2023", "2024", "2025", "2026"];
 ...
 const renderPaginationButtons = () => {
    ...
    <DashboardLayout title="Manajemen Data">
      <div className="w-full relative">
        <div className="mb-5 flex items-start justify-between">
          <div>
            <h1 className="text-[26px] font-bold text-gray-900">Manajemen Data</h1>
            <p className="text-[#9CA3AF] text-sm mt-1">Kelola validasi dan kirim data mahasiswa ke Tata Usaha Fakultas</p>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => setShowImportModal(true)} className="h-10 px-5 rounded-lg bg-white border border-gray-200 shadow-sm text-sm font-semibold text-gray-700 flex items-center gap-2">
              <FiUpload size={16} /> Import Data
            </button>
          </div>
        </div>

        {/* FILTER SECTION - SEPERTI IJAZAH TERBIT */}
          ...
          <div className="flex items-center gap-3 ml-auto justify-end">
            <div className="relative">
              <select
                ...
                className="appearance-none bg-[#E5E5E5] text-sm font-semibold text-gray-700 px-4 h-11 rounded-lg pr-10 min-w-[220px] outline-none cursor-pointer"
              >
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none bg-[#E5E5E5] text-sm font-semibold text-gray-700 px-4 h-11 rounded-lg pr-10 min-w-[150px] outline-none cursor-pointer"
              >
                {tahunList.map((t, i) => (
        ...
        {/* TABLE SECTION - DENGAN OVERFLOW UNTUK SCROLL */}
            ...
            <table className="w-full text-sm text-left whitespace-nowrap">
              <thead className="bg-[#F3F4F6] text-gray-500 font-bold border-b border-gray-200">
                <tr>
                  ...
                  <th className="py-4 px-6 text-center min-w-[220px]">Fakultas</th>
                  <th className="py-4 px-6 text-center min-w-[180px]">Program Studi</th>
  ...
  {paginatedData.map((item, index) => (
    <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
      <td className="py-4 px-6 text-center font-medium text-gray-800">{(currentPage - 1) * itemsPerPage + index + 1}.</td>
      <td className="py-4 px-6 font-medium text-gray-900">{item.nama}</td>  
      <td className="py-4 px-6 text-center font-medium text-gray-900">{item.nim}</td>
      <td className="py-4 px-6 text-center text-gray-600 whitespace-normal break-words max-w-[220px]">{item.fakultas}</td>
      <td className="py-4 px-6 text-center text-gray-600 whitespace-normal break-words max-w-[180px]">{item.prodi}</td>
      <td className="py-4 px-6 text-center font-medium text-gray-900">{item.tahunLulus}</td>
      <td className="py-4 px-6 text-center">
        <div onClick={() => handleDetailClick(item)} className="w-8 h-8 border border-gray-300 rounded-md flex items-center justify-center mx-auto cursor-pointer hover:bg-gray-200 transition">
          ...
          {/* PAGINATION - SEPERTI IJAZAH TERBIT */}
          {totalPages > 0 && (
        ...
        {showImportModal && <ImportDataModal onClose={() => setShowImportModal(false)} />}
      </div>
    </DashboardLayout>
  );
};
