...
const DetailDokumenValidRektor = () => {
  ...
  const fetchDetailDokumen = async () => {
    ...
    try {
      const result = await getValidDocumentBatchDetail(activeBatchId, {
        search,
      });
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Gagal mengambil detail dokumen valid.",
      );
      setMahasiswa([]);
    } finally {
      setIsLoading(false);
  ...
  const handleGoDetailMahasiswa = (student) => {
    navigate(`/rektor/detail-mahasiswa/${student.nim}`, {
      state: student,
    });
  };
  ...
  return (
    <DashboardLayout title="Dokumen Valid">
      <div className="w-full pb-10">
        ...
        {(batch?.batch || batch?.nomor_batch_upload) && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-6 flex flex-wrap gap-6">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-[#117065]">
                {batch.total || mahasiswa.length || 0}
              </p>
            </div>
          </div>
        )}
        ...
        <div
          ref={searchRef}
          className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-0"
        >
            ...
            {searchSuggestions.length > 0 ? (
              searchSuggestions.map((student) => (
                <div
                  ...
                  onClick={() => {
                    setShowSuggestions(false);
                    handleGoDetailMahasiswa(student);
                  }}
                ...
                >
                  <div className="flex flex-col gap-0.5">
                    ...
                    <div className="text-[12px] font-normal text-gray-500">
                      {student.nim || "-"} •{" "}
                      {student.prodi || student.program_studi || "-"}
                    </div>
                  </div>
                        ...
                        <div className="flex items-center justify-center gap-2">
                          {item.ijazah?.file_pdf_url && (
                            <button
                              type="button"
                              onClick={() =>
                                openPdf(item.ijazah.file_pdf_url)
                              }
                          ...
                          {item.transkrip?.file_pdf_url && (
                            <button
                              type="button"
                              onClick={() =>
                                openPdf(item.transkrip.file_pdf_url)
                              }
