...
import { decodeId } from "@/components/shared/hashId"; // ✅ Import helper
...
const DetailMahasiswa = () => { // ✅ Ganti nama komponen
  const navigate = useNavigate();
  const { nim: encodedNim } = useParams();

  // ✅ Decode NIM dari URL
  const nim = decodeId(encodedNim);
  ...
  // ✅ Validasi: jika NIM hasil decode tidak valid, tendang
  if (!nim) {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setError(err?.message || err?.response?.data?.message || "Gagal mengambil detail mahasiswa.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const batchLabel = batch?.id_batch_upload || mahasiswa?.id_batch_upload || "-";
  const detailStatus = approval?.status || "Proses";
  const detailKeterangan = approval?.keterangan || "Di Proses Validasi oleh TU Fakultas";
  const detailDeskripsi = approval?.deskripsi || "Data sedang dalam proses verifikasi. Mohon menunggu hingga proses validasi selesai.";
      ...
      <div className="w-full">
        <div className="bg-white rounded-xl px-8 py-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 shadow-sm border border-gray-200">
          ...
          <div className="text-right flex flex-col items-end gap-1 max-w-[320px]">
            <span className={`${badgeClass(detailStatus)} text-white text-[13px] px-6 py-1.5 rounded-full font-bold shadow-sm inline-block`}>
              {detailStatus}
            </span>
            <p className="text-[11px] text-gray-500 font-medium">{detailKeterangan}</p>
            <p className="text-[10px] text-gray-400 italic leading-relaxed text-right">{detailDeskripsi}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
            ...
            <div className="p-6 grid grid-cols-2 gap-y-6 gap-x-8 text-[14px]">
              ...
              <InfoItem label="Tempat, Tanggal Lahir" value={`${mahasiswa?.tempat_lahir || "-"}, ${formatTanggal(mahasiswa?.tanggal_lahir)}`} />
              ...
              <InfoItem label="Nomor Seri Ijazah" value={mahasiswa?.nomor_seri_ijazah} />
              ...
              <tbody>
                {transkrip.length > 0 ? (
                  transkrip.map((n, index) => (
                    <tr key={`${n.kode || "matkul"}-${index}`} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 font-semibold text-center">{n.kode || "-"}</td>
                      <td className="px-6 py-4 font-semibold">{n.nama || "-"}</td>
                      <td className="px-6 py-4 font-semibold text-center">{n.k ?? "-"}</td>
                      <td className="px-6 py-4 font-semibold text-center">{n.am ?? "-"}</td>
                      <td className="px-6 py-4 font-semibold text-center">{n.t ?? "-"}</td>
