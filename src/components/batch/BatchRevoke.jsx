...
const formatMahasiswa = (item, index, batchData) => {
  return {
    ...
    status: item.status || batchData?.status || "Revoke",
  ...
  useEffect(() => {
    ...
    const fetchBatchRevoke = async () => {
      try {
        ...
        const revokeRows = rows.filter(
          (item) => normalizeStatus(item.status) === "revoke"
        );

        const selectedRows = revokeRows.filter((item) => {
        ...
        console.error("Gagal mengambil detail batch revoke:", error);
            ...
            <tbody>
              {sortedMahasiswa.length > 0 ? (
                ...
                <tr>
                  ...
                  >
                    Data mahasiswa revoke tidak ditemukan.
                  </td>
                </tr>
              )}
            </tbody>
