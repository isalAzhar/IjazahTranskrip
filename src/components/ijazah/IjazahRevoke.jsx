...
const buildFakultasOptions = (rows = []) => {
  const uniqueFakultas = [
    ...new Set(
      rows
        .filter((item) => normalizeStatus(item.status) === "revoke")
  ...
  useEffect(() => {
    const fetchIjazahRevoke = async () => {
      try {
        ...
        console.error("Gagal mengambil data ijazah revoke:", error);
