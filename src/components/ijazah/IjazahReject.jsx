...
const IjazahReject = () => {
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      setIsLoading(true);
      try {
        const url = new URL("https://URL_DARI_BACK_END/api/ijazah-reject");
  ...
  return (
    <DashboardLayout>
      <div className="w-full">
        {/* HEADER YANG SUDAH DIREVISI (SESUAI FIGMA) */}
                ...
                return (
                  <tr key={i} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-6 text-center">
                      <span className="inline-block bg-[#DC2626] text-white px-4 py-1.5 rounded-full text-xs font-bold tracking-wide shadow-sm">
                        {item.status}
                      </span>
                    </td>

                    {/* MENGARAH KE BATCH REJECT */}
                    <td className="py-4 px-6 text-center">
                      <div
                        onClick={() => navigate(`/batch-reject/${actualIndex}`, { state: item })}
