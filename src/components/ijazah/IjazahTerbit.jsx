...
const IjazahTerbit = () => {
      ...
      batch.mahasiswa.forEach((mhs) => {
        ...
        if (matchNama || matchNim || matchProdi) {
          result.push({
            ...
            mahasiswa: { ...mhs, batch: mhs.batch || batch.batch, fakultas: mhs.fakultas || batch.fakultas, tahun: mhs.tahun || batch.tahun, status: "Terbit" },
          });
        }
      });
    });
  ...
  const handleDetailBatch = (item, actualIndex) => {
    navigate(`/batch/terbit/${actualIndex}`, { state: item });
  };

  // ✅ PAGINATION STYLE SAMA PERSIS DENGAN DataMahasiswa
  ...
  return (
    <DashboardLayout>
      <div className="w-full">
        <div className="mb-6">
          <h1 className="text-[26px] font-bold text-gray-900">Jumlah Ijazah Terbit</h1>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                return (
                  <tr key={`${item.batch}-${item.fakultas}-${item.tahun}-${item.periode}`} className="h-[70px] border-t border-gray-200 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center align-middle">
                      <button
                        type="button"
                        onClick={() => handleDetailBatch(item, actualIndex)}
          ...
          {/* PAGINATION - SAMA PERSIS DENGAN DataMahasiswa */}
